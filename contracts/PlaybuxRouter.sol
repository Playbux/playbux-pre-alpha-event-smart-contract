...
contract PlaybuxRouter is ReentrancyGuard, Ownable, Pausable {
    ...
    function swap(
        ...
        require(knownToken[IERC20(path[0])], "Unknown token");
        ...
        uint256 outputDecimals = ERC20(path[1]).decimals();
        ...
        uint256 feeAmount = (amounts[1] * feeWithDecimals) /
            (10**outputDecimals);
        console.log("feeAmount", feeAmount);
        uint256 receivedAmount = amounts[1] - feeAmount;
        console.log("receivedAmount", receivedAmount);

        IERC20(path[1]).safeTransfer(to, receivedAmount);
    }

    function adjustFee(uint256 _fee) external onlyOwner {
        require(
            _fee <= 0.05 ether,
            "Fee must be less than or equal to 5 percent"
        );
        fee = _fee;
    }
